...
int main()
{
  ...
  ////////////////////////////////
  //int projectionNum=128;
  int projectionNum=64;
  ...
  // CT simulation
  ////////////////////////////////
  long long int simCount =20000000;
  ...
  for(int proj=0; proj<projectionNum; proj++){
    ...
    //montecarlo sim air
    SharedDetectorPtr airDetector =Detector::Create(detParam);
    for(long long int i=0; i<simCount; i++){

      RunLight(i, simCount);
      SharedFlyingPhotonPtr photon =pg->GeneratePhoton(random);
      airDetector->Detect(photon);
    }


    //log conversion
    std::vector<double> det, air, result;
    detector->Export(det);
    airDetector->Export(air);

    for(int i=0; i<det.size(); i++){
      det[i] +=0.00001;
      air[i] +=0.000001;
      result.push_back( std::max(-log(det[i]/air[i]), 0.0) );
    }
    ...
    //export result
    {
      ...
      ss >> outpath;
      //SavePixelDoubleToInt(outpath, result, 100.0);
    }


    //export result to PGM
    {
      std::string outpath;
      std::stringstream ss;
      ss << "projection_"<< detParam.SizeU << "x" << detParam.SizeV << "_" << proj << ".pgm"; 
      ss >> outpath;
      SaveCropPixelDoubleToPGM(outpath, result, detParam.SizeU, detParam.SizeV, 100.0, 256);
    }
